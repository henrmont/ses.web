...
export class HaltedPatientRequestComponent {
  ...
  /**
   * Dispara a requisição para paralisar/sobrestar a solicitação do paciente
   */
  protected onSubmit(): void {
    ...
    this.patientRequestService.haltedPatientRequest(requestId)
      .pipe(
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.dialogRef.close(true); // Sinaliza sucesso à listagem pai para recarregar a grid
        },
        error: (err) => {
          const errorMessage = err?.error?.message || 'Ocorreu um erro operacional ao tentar paralisar a solicitação.';
