...
describe('MovePatientRequestFromOthersComponent', () => {
  ...
  // Mock estruturado exatamente igual ao seu HTML de solicitações de paciente
  ...
  describe('Fluxo de Submissão (onSubmit)', () => {
    it('deve barrar a execução e exibir mensagem de erro se o id da solicitação não estiver presente', () => {
        ...
        .overrideComponent(MovePatientRequestFromOthersComponent, {
          set: {
            providers: [
              ...
              { provide: MessageService, useValue: messageServiceMock },
              { provide: MatDialogRef, useValue: dialogRefMock },
              { provide: MAT_DIALOG_DATA, useValue: { patient_request: { id: null, report: { patient_care: { name: 'Incompleto' } } } } }
            ]
          }
        });
      ...
      expect(patientRequestServiceMock.movePatientRequestFromOthers).not.toHaveBeenCalled();
    });
    ...
    it('deve movimentar a solicitação com sucesso, exibir toast e fechar a modal retornando true', () => {
      ...
      patientRequestServiceMock.movePatientRequestFromOthers.mockReturnValue(of(mockApiResponse));

      component['onSubmit']();

      expect(patientRequestServiceMock.movePatientRequestFromOthers).toHaveBeenCalledWith(740);
    ...
    it('deve usar mensagem padrão de sucesso se a API retornar um objeto vazio', () => {
      fixture.detectChanges();
      patientRequestServiceMock.movePatientRequestFromOthers.mockReturnValue(of({}));
    ...
    it('deve tratar falhas do servidor, exibir mensagem de erro da API e resetar o estado de carregamento do botão', () => {
      ...
      patientRequestServiceMock.movePatientRequestFromOthers.mockReturnValue(throwError(() => mockApiError));
    ...
    it('deve usar mensagem de erro genérica se o servidor falhar sem retornar corpo de erro', () => {
      ...
      patientRequestServiceMock.movePatientRequestFromOthers.mockReturnValue(throwError(() => rawError));
