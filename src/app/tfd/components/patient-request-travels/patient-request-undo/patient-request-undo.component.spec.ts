...
describe('UndoPatientRequestComponent', () => {
  ...
  describe('Fluxo de Submissão e Retorno do Pedido (onSubmit)', () => {
    beforeEach(() => {
      ...
      // Limpa validações para isolamento perfeito de testes de submissão baseados em estados forçados
      ...
      // Redefine propriedades de valid/invalid para os mocks manuais
    ...
    it('deve barrar a submissão marcando controles como touched se o formulário estiver inválido', () => {
      const tramitForm = component['tramitPatientRequestForm'];
      // Espiona o método de toque em massa do formulário reativo
      const markAllAsTouchedSpy = vi.spyOn(tramitForm, 'markAllAsTouched');
      
      // Força o formulário a ficar inválido
      Object.defineProperty(tramitForm, 'invalid', { get: () => true });
    ...
    it('deve barrar a submissão e exibir alerta se o ID da solicitação do paciente não existir nos dados da modal', () => {
      ...
      Object.defineProperty(component['tramitPatientRequestForm'], 'invalid', { get: () => false });
