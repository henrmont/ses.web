...
describe('MovePatientRequestFromHistoryComponent', () => {
  ...
  beforeEach(async () => {
    costAssistanceServiceMock = {
      movePatientRequestFromHistory: vi.fn()
    };
    ...
    costAssistanceServiceMock.movePatientRequestFromHistory.mockReturnValue(
  ...
  describe('Fluxo de Execução - onSubmit()', () => {
    ...
    it('deve exibir mensagem de erro se o id do request não for informado', () => {
      ...
      expect(costAssistanceServiceMock.movePatientRequestFromHistory).not.toHaveBeenCalled();
      expect(component['isSubmitting']()).toBe(false);
    });
    ...
    it('deve movimentar a solicitação passando apenas o id correto e fechar a modal', () => {
      ...
      expect(costAssistanceServiceMock.movePatientRequestFromHistory).toHaveBeenCalledWith(880);
    ...
    it('deve usar mensagem de fallback no sucesso se a API não retornar uma mensagem explícita', () => {
      costAssistanceServiceMock.movePatientRequestFromHistory.mockReturnValue(of({}));
      component['data'].patient_request.id = 880;
