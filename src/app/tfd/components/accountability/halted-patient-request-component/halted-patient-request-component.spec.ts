...
describe('HaltedPatientRequestComponent', () => {
  ...
  describe('Fluxo de Submissão (onSubmit)', () => {
    it('deve barrar a execução e exibir mensagem de erro se o id da solicitação não estiver presente', () => {
      ...
      expect(accountabilityServiceMock.haltedPatientRequest).not.toHaveBeenCalled();
    });

    // ==========================================
    // 🎯 FLUXO A: MARCAR SOBRESTADO (is_accountability_bookmark: false)
    // ==========================================
    describe('Quando a ação for MARCAR SOBRESTADO (is_accountability_bookmark: false)', () => {
      it('deve paralisar a solicitação com sucesso, exibir mensagem da API e fechar a modal retornando true', () => {
        ...
        accountabilityServiceMock.haltedPatientRequest.mockReturnValue(of(mockApiResponse));

        component['onSubmit']();

        expect(component['isSubmitting']()).toBe(false);
        expect(accountabilityServiceMock.haltedPatientRequest).toHaveBeenCalledWith(450);
      ...
      it('deve usar mensagem padrão de sucesso se a API retornar um objeto vazio', () => {
        fixture.detectChanges();
        accountabilityServiceMock.haltedPatientRequest.mockReturnValue(of({}));

        component['onSubmit']();

        // 100% em português agora:
      ...
      it('deve tratar falhas do servidor, exibir mensagem de erro da API e liberar o estado de submissão', () => {
        fixture.detectChanges();
        const mockApiError = { error: { message: 'Não é possível paralisar uma solicitação com prestações homologadas.' } };
        accountabilityServiceMock.haltedPatientRequest.mockReturnValue(throwError(() => mockApiError));
      ...
      it('deve usar mensagem de erro genérica se o servidor falhar sem retornar corpo de erro', () => {
        ...
        accountabilityServiceMock.haltedPatientRequest.mockReturnValue(throwError(() => rawError));
    ...
    // ==========================================
    // 🎯 FLUXO B: DESMARCAR SOBRESTADO (is_accountability_bookmark: true)
    // ==========================================
    describe('Quando a ação for DESMARCAR SOBRESTADO (is_accountability_bookmark: true)', () => {
      beforeEach(() => {
        // Altera o valor em tempo de execução contornando a restrição estrutural do objeto
        Object.assign(component['data'].patient_request, { is_accountability_bookmark: true });
      });

      it('deve executar a chamada de API normalmente repassando o ID da solicitação', () => {
        fixture.detectChanges();
        accountabilityServiceMock.haltedPatientRequest.mockReturnValue(of({ message: 'Retornado ao fluxo normal.' }));
      ...
      it('deve reter o comportamento e resetar o sinal isSubmitting caso ocorra erro no fluxo de desmarcação', () => {
        fixture.detectChanges();
        accountabilityServiceMock.haltedPatientRequest.mockReturnValue(throwError(() => ({ status: 400 })));
