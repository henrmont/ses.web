...
export class HaltedPatientRequestComponent {
  ...
  /**
   * Dispara a requisição para paralisar/sobrestar a solicitação de prestação de contas
   */
  protected onSubmit(): void {
    ...
    this.accountabilityService.haltedPatientRequest(requestId)
      .pipe(
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.dialogRef.close(true); // Sinaliza sucesso para atualizar a grid principal de Prestação de Contas
        },
        error: (err) => {
