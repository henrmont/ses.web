...
@Component({
  selector: 'app-finish-back-patient-component',
  standalone: true,
  imports: [
    CommonModule, 
    MatDialogModule, 
  ...
  templateUrl: './finish-back-patient-component.html',
  styleUrl: './finish-back-patient-component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FinishBackPatientComponent {
  // Injeções de Dependência Dinâmicas via inject()
  protected readonly data = inject(MAT_DIALOG_DATA);
  private readonly patientService = inject(PatientService);
  private readonly messageService = inject(MessageService);
  private readonly dialogRef = inject(MatDialogRef<FinishBackPatientComponent>);
